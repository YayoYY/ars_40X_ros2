...
namespace ars_40X {
ClusterListROS::ClusterListROS(rclcpp::Node& nh, ARS_40X_CAN *ars_40X_can, std::string port_id) :
    ars_40X_can_(ars_40X_can), cluster_id_(0), node_(nh) {
  ...
  clusters_data_pub_ = node_.create_publisher<perception_ros2_msgs::msg::ClusterList>("ars_40X/clusters" + port_id, 10);         
}
...
void ClusterListROS::send_cluster_1_general() {
  perception_ros2_msgs::msg::Cluster cluster;
...
void ClusterListROS::send_cluster_2_quality() {
  if (cluster_id_ >= cluster_list.clusters.size()) {
    return;
  }
