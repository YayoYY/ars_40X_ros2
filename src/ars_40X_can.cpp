...
void copy_can_payload(uint8_t (&raw_data)[N], const uint8_t *data, uint8_t dlc) {
  std::memset(raw_data, 0, N);
  std::memcpy(raw_data, data, std::min<std::size_t>(N, dlc));
}
}
...
bool ARS_40X_CAN::receive_radar_data() {
  ...
  // printf("read_status = %d, frame_id = %d\n", read_status, frame_id);
  if (!read_status) {
    return false;
  }
  switch (frame_id) {
    case RadarState:copy_can_payload(radar_state_.get_radar_state()->raw_data, data, dlc);
      send_radar_state();
      // printf("RadarState\n");
      break;

    case Cluster_0_Status:copy_can_payload(cluster_0_status_.get_cluster_0_status()->raw_data, data, dlc);
      send_cluster_0_status();
      // printf("Cluster_0_Status\n");
      break;

    case Cluster_1_General:copy_can_payload(cluster_1_general_.get_cluster_1_general()->raw_data, data, dlc);
      send_cluster_1_general();
      // printf("Cluster_1_General\n");
      break;

    case Cluster_2_Quality:copy_can_payload(cluster_2_quality_.get_cluster_2_quality()->raw_data, data, dlc);
      send_cluster_2_quality();
      // printf("Cluster_2_Quality\n");
      break;

    case Object_0_Status:copy_can_payload(object_0_status_.get_object_0_status()->raw_data, data, dlc);
      send_object_0_status();
      // printf("Object_0_Status\n");
      break;

    case Object_1_General:copy_can_payload(object_1_general_.get_object_1_general()->raw_data, data, dlc);
      send_object_1_general();
      // printf("Object_1_General\n");
      break;

    case Object_2_Quality:copy_can_payload(object_2_quality_.get_object_2_quality()->raw_data, data, dlc);
      send_object_2_quality();
      // printf("Object_2_Quality\n");
      break;

    case Object_3_Extended:copy_can_payload(object_3_extended_.get_object_3_extended()->raw_data, data, dlc);
      send_object_3_extended();
      // printf("Object_3_Extended\n");
      break;
